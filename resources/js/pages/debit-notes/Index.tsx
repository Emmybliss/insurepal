...
export default function DebitNotesIndex({ notes, customers, filters, stats }: Props) {
    const [search, setSearch] = useState(filters.search || '');
    const [status, setStatus] = useState(filters.status || '');
    const [customerId, setCustomerId] = useState(filters.customer_id || '');

    const handleSearch = () => {
        router.get(route('debit-notes.index'), { search, status, customer_id: customerId }, { preserveState: true, replace: true });
    };
    ...
    const clearFilters = () => {
        ...
        setStatus('');
        setCustomerId('');
        router.get(route('debit-notes.index'));
    };

    const getStatusIcon = (note: DebitNote) => {
        if (note.status === 'paid') {
    ...
    const getStatusColor = (note: DebitNote) => {
        if (note.status === 'paid') {
            ...
            <div className="flex-1 space-y-4 pt-4">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground">Manage your Debit Notes</p>
                    </div>
                    <div>
                        <Link href={route('debit-notes.create')}>
                            <Button>
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-red-600">{formatCurrency(stats.total_debit)}</div>
                            <p className="text-xs text-muted-foreground">All debit notes</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-yellow-600">{formatCurrency(stats.outstanding_debit)}</div>
                            <p className="text-xs text-muted-foreground">Unpaid debit notes</p>
                        </CardContent>
                    </Card>
                    ...
                    <CardContent>
                        {notes.data.length > 0 ? (
                                                ...
                                                <td className="px-4 py-3">
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            ...
                                                            <Link href={route('debit-notes.download', note.id)}>
                                                                <DropdownMenuItem>
                                                                    <Download className="mr-2 h-4 w-4" />
                                                                    Download PDF
                                                                </DropdownMenuItem>
                                                            ...
                                                            {note.status === 'draft' && (
                                                                <DropdownMenuItem
                                                                    ...
                                                                    onClick={() => router.delete(route('debit-notes.destroy', note.id))}
                                                                >
                            ...
                            <div className="py-8 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-semibold text-gray-600">No debit notes found</h3>
                                <p className="mb-4 text-gray-500">Create your first debit note to get started.</p>
                                <Link href={route('debit-notes.create')}>
                                    <Button>
                                        <PlusCircle className="mr-2 h-4 w-4" />
                                        Create Debit Note
                        ...
                        <div className="flex space-x-2">
                            {notes.current_page > 1 && (
                                <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => router.get(route('debit-notes.index'), { page: notes.current_page - 1 }, { preserveState: true })}
                                >
                                    Previous
                                </Button>
                            )}
