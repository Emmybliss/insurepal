...
export default function CreateDirect({ customers, policyProducts }: Props) {
    ...
    // Update related fields when product is selected
    useEffect(() => {
        if (selectedProduct) {
            setData((prevData) => ({
                ...prevData,
    ...
    // Sync local date object from string when form state changes externally
    useEffect(() => {
    ...
    // Calculate premium based on sum assured
    useEffect(() => {
        if (selectedProduct && data.coverage_details.sum_assured) {
            ...
            const rate = basePremium / 100000; // Assuming base premium is per 100k sum assured
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('policy-management.store-direct'), {
            ...
            onError: (errors) => {
                console.log('CreateDirect Data', errors);
                toast.error('Error creating policy. Please check the form for errors.');
            },
        });
    };
    ...
    return (
        <AppSidebarLayout>
            <Head title="Create Direct Policy" />

            <div className="flex flex-1 flex-col space-y-8 p-8">
                <div className="flex items-center justify-between space-y-2">
                    <div className="flex items-center space-x-4">
                        <div>
                            <h2 className="text-2xl font-bold tracking-tight">Create Direct Policy</h2>
                            <p className="text-muted-foreground">Issue a new insurance policy directly</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="grid gap-8 md:grid-cols-2">
                                ...
                                {selectedProduct && (
                                    <div className="rounded-lg bg-muted p-4">
                                        ...
                                        <div className="space-y-1 text-sm">
                                            ...
                                            <p>
                                                <strong>Commission Rate:</strong> {selectedProduct.commission_rate}%
                                            </p>
                                        </div>
                                    </div>
                                )}
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Set the policy effective and expiry dates</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                <div className="space-y-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    {selectedProduct && (
                                        <p className="text-xs text-muted-foreground">
                                            Range: {formatCurrency(selectedProduct.min_sum_assured)} -{' '}
                                            {formatCurrency(selectedProduct.max_sum_assured)}
                                        </p>
                                    )}
                                </div>
