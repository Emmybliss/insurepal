...
export default function ShowCreditNote({ note: noteFromProps }: NoteDetailsProps) {
    const [showPreviewModal, setShowPreviewModal] = useState(false);
    const [showIssueModal, setShowIssueModal] = useState(false);
    const [showCancelModal, setShowCancelModal] = useState(false);
    const note = { ...noteFromProps, type: 'credit' as const };
    ...
    const handleIssue = (e: React.FormEvent) => {
        e.preventDefault();
        postIssue(route('credit-notes.issue', note.id), {
            onSuccess: () => {
            ...
            onError: () => {
                console.log('Failed to issue credit note.');
    ...
    const handleCancel = (e: React.FormEvent) => {
        e.preventDefault();
        postCancel(route('credit-notes.cancel', note.id), {
            onSuccess: () => {
            ...
            onError: () => {
                console.log('Failed to cancel credit note.');
    ...
    const handleRegenerate = () => {
        ...
        router.visit(route('credit-notes.template-options', note.id), {
            data: {
                template_id: note.document_template_id,
                regenerate_credit_note_id: note.id,
            },
        });
    };
    return (
        <AppLayout>
            <Head title={`Credit Note - ${note.note_number}`} />

            <div className="flex-1 space-y-4 pt-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-3xl font-bold tracking-tight">Credit Note Details</h2>
                        <p className="text-muted-foreground">View credit note details</p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        {note.status === 'draft' && (
                            <>
                                ...
                                <Button onClick={() => router.post(route('credit-notes.issue', note.id), {})}>Issue Note</Button>
                            </>
                        )}
                        {note.status === 'issued' && (
                            <Button
                                onClick={() =>
                                    router.post(
                                        route('credit-notes.mark-paid', note.id),
                                        {},
                                        {
                                            onSuccess: () => {
                                                toast.success('Credit note marked as paid successfully.');
                                            },
                                            onError: () => {
                                                console.log('Failed to mark credit note as paid.');
                                                toast.error('Failed to mark credit note as paid.');
                                            },
                                        },
                                    )
                                }
                        ...
                        {note.file_path && (
                            <Button variant="outline" onClick={() => (window.location.href = route('credit-notes.download', note.id))}>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <Card className="md:col-span-2">
                        ...
                        <CardFooter className="flex justify-between">
                            <Button variant="outline" onClick={() => router.get(route('credit-notes.index'))}>
                                Back to List
                            </Button>
                            {note.status === 'draft' && (
                                <Button variant="destructive" onClick={() => router.delete(route('credit-notes.destroy', note.id))}>
                                    Delete Note
                                </Button>
                            )}
                        </CardFooter>
            ...
            {showIssueModal && (
                <div className="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
                    <div className="w-full max-w-md rounded-lg bg-white p-6 dark:bg-gray-800">
                        <h3 className="mb-4 text-lg font-semibold">Issue Credit Note</h3>
                        <form onSubmit={handleIssue}>
                            <div className="mb-4">
                                ...
                                <Textarea
                                    ...
                                    placeholder="Add any notes about issuing this credit note..."
                                    rows={3}
                                    className="mt-1"
                                />
                            </div>
                            <div className="flex gap-3">
                                <Button type="submit" disabled={issueProcessing} className="flex-1">
                                    {issueProcessing ? 'Issuing...' : 'Issue Credit Note'}
                                </Button>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
                    <div className="relative w-full max-w-5xl rounded-lg bg-white shadow-xl dark:bg-gray-800">
                        <div className="flex items-center justify-between border-b px-4 py-2">
                            <h3 className="text-lg font-semibold">Credit Note Preview</h3>
                        ...
                        <div className="flex h-[90vh] justify-center overflow-auto bg-gray-50 p-4 dark:bg-gray-600">
                            {note.file_path && <iframe src={route('credit-notes.preview', note.id)} className="h-full w-full rounded border" />}
                        </div>
                    </div>
                </div>
            )}
            ...
            {showCancelModal && (
                <div className="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black p-4">
                    <div className="w-full max-w-md rounded-lg bg-white p-6 dark:bg-gray-800">
                        <h3 className="mb-4 text-lg font-semibold">Cancel Credit Note</h3>
                        <form onSubmit={handleCancel}>
                            <div className="mb-4">
                                ...
                                <Textarea
                                    ...
                                    placeholder="Please provide a reason for cancelling this Credit Note..."
                                    rows={3}
                                    className="mt-1"
                                    required
                                />
                            ...
                            <div className="flex gap-3">
                                <Button type="submit" variant="destructive" disabled={cancelProcessing} className="flex-1">
                                    {cancelProcessing ? 'Cancelling...' : 'Cancel Credit Note'}
                                </Button>
