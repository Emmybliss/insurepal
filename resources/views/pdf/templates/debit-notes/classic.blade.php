...
@section('content')
        ...
        <table style="width: 100%; border-collapse: collapse; border: none; table-layout: fixed;">
            <tr>
                <td style="vertical-align: middle; text-align: left;">
                    <h1 style="margin: 0; color: {{ $branding['primary_color'] ?? '#333' }}; font-size: 20px; text-transform: uppercase;">{{ $labels['title_label'] ?? 'Debit Note' }}</h1>
        ...
        <table style="width: 100%; border-collapse: collapse; border: none; table-layout: fixed;">
            <tr>
                <td style="vertical-align: top; text-align: left; width: 60%;">
                    <h3 style="color: #777; margin: 0 0 4px 0; font-size: 11px; text-transform: uppercase; letter-spacing: 1px;">{{ $labels['recipient_label'] ?? 'Insured:' }}</h3>
                ...
                <td style="vertical-align: top; text-align: right; width: 40%;">
                    ...
                    @if(!empty($payload['insurer_name']) && $payload['insurer_name'] !== 'N/A')
                        <div style="background: #f9f9f9; padding: 4px 10px; border-left: 3px solid #666; display: inline-block; text-align: left;">
    ...
    <table style="width: 100%; margin-top: 15px; border-collapse: collapse;">
        ...
        <tbody>
            <tr>
                @if(!empty($payload['description']))<td style="padding: 8px 0; font-size: 12px; text-align: left;">{{ $payload['description'] }}</td> @else <td style="padding: 8px 0; font-size: 12px; text-align: left;">Being premium due on the Policy for the period {{ $payload['effective_date'] }} to {{ $payload['expiry_date'] }}</td> @endif
