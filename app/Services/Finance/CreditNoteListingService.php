...
class CreditNoteListingService
{
    use HandlesListing;
    ...
    {
        $nextNoteNumber = CreditNote::generateCreditNoteNumber($user->tenant_id);
    ...
    {
        $base = CreditNote::where('tenant_id', $user->tenant_id);

        return [
            'total_credit' => (clone $base)->sum('total_amount'),
            'outstanding_credit' => (clone $base)->where('status', CreditNote::STATUS_ISSUED)->sum('total_amount'),
